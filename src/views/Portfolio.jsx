  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        const myProjectsColaborator = response.data.data.filter((project) =>
          project.collaborators.some((rol) =>
            rol.users.some(
              (user) => user._id === id && project.status === "Closed"
            )
          )
        );
      ...
      <div id="portfolio-view">
        <div>
          ...
          {projectsColaborator.length === 0 && (
            <p style={{ color: "black", fontSize: 20, paddingTop: 25 }}>
              {" "}
              ⚠️ There is not projects closed as a collaborator.
            </p>
          )}
          {projectsColaborator.length !== 0 &&
        ...
        <div>
          ...
          {projectsTL.length === 0 && (
            <p style={{ color: "black", fontSize: 20, paddingTop: 25 }}>
              {" "}
              ⚠️ There is not projects closed as a team lead.
            </p>
          )}
          {projectsTL.length !== 0 &&
